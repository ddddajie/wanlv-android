...
class DigitalHumanSessionManager(
    ...
    suspend fun connect(baseUrl: String, onVideoFrame: (Bitmap) -> Unit): Long {
        ...
        val observer = DigitalHumanPeerObserver(
            ...
            onTrack = { track ->
                when (track) {
                    is VideoTrack -> bindVideoTrack(track, onVideoFrame)
        ...
        // 重点：等待 ICE 候选写入本地 SDP 后再发给数字人服务，避免只发半成品 offer。
        withTimeoutOrNull(3000) { iceGatheringComplete.await() }
        val localSdp = peer.localDescription?.description ?: offer.description
        val answer = repository.offer(baseUrl = baseUrl, sdp = localSdp)
        if (answer.sdp.isBlank() || answer.sessionId == 0L) {
            error("数字人 /offer 返回内容不完整")
        }
    ...
    private fun bindVideoTrack(track: VideoTrack, onVideoFrame: (Bitmap) -> Unit) {
    ...
    private fun disconnectPeerOnly() {
        videoSink?.let { oldSink ->
            remoteVideoTrack?.removeSink(oldSink)
            oldSink.release()
        }
